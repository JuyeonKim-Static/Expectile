...
tau_vec <- c(1:9)/10 
set.seed(230430)# artibrarily chosen; should be changed at every simulation
...
for(i in 1:3){
  for(j in 1:9){
      ...
      coverage_tau <- foreach(c = 1:1000, .combine = rbind, .packages = c("hdrcde", "mvtnorm", "quantreg")) %dopar% {
        mvn.dat <- as.data.frame(mvtnorm::rmvnorm(n, sigma = matrix(c(1, rho, rho, 1), ncol = 2)))
        names(mvn.dat) <- c("x", "y")
        I <- ifelse((mvn.dat$x <= c) & (mvn.dat$y <= c) , 1 ,0 )
        mvn.dat1 <- I* rnorm(n, mean = 0, sd = 1)
        mvn_x <- mvn.dat$x+ mvn.dat1
        mvn_y <- mvn.dat$y+ mvn.dat1
      ...
      cat ("Simulation: DGP2, multivariate-(Rocket type)", 
           "n: ", n, "tau:",tau ,"rho: " , rho ,"\n",
      ...
      result_obj[[simul_num]] <- list(obj = coverage_tau, 
                                      description = paste("Simulation: DGP2, multivariate-(Rocket type)", 
                                                          "n: ", n, "tau:",tau ,"rho: " , rho ,"\n",
                                                         "coverage (quantile corr):", sum(coverage_tau[,3])/1000 ,"\n",
                                                         "coverage (expectile corr):", sum(coverage_tau[,6])/1000 ,"\n",
                                                         difftime(time2, time1, unit = "mins"), "mins", 
      ...
      saveRDS(result_obj, paste0("/Users/gimjuyeon/Documents/학부연구생/연구/coverage_result_DGP2(multi-Rocket type).RDS"))
